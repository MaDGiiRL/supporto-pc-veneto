...
class CoordinamentoController extends Controller
{
    ...
    // Ruoli esposti al client (puoi leggere da DB/ACL reali se vuoi)
    ...
    // Lista compatibile col frontend (filtri: status, assigned_to)
    public function listSegnalazioni(Request $r)
    {
        $status = $r->string('status');         // aperta|in_lavorazione|chiusa
        $ass    = $r->string('assigned_to');    // opzionale
        ...
        $q = SegnalazioneGenerica::query()
            ...
            ->orderByDesc('creata_il')->orderByDesc('id');
        ...
        // Il tuo frontend accetta sia array puro che {data:[...]}
        return response()->json($rows);
    }

    // Smista (solo admin/coordinamento)
    public function assign(Request $r, int $id)
    {
        $me = $r->user();
        $role = $me->role ?? 'coordinamento';
        if ($role !== 'coordinamento') return response()->json(['message' => 'Operazione non permessa'], 403);

        $to = (string) $r->input('to'); // volontariato|mezzi|prociv
    ...
    public function addNote(Request $r, int $id)
    {
        ...
        DB::transaction(function () use ($id, $me, $text) {
            SorNote::create([
                'segnalazione_id' => $id,
                'text'            => $text,
                'created_by'      => $me->name ?? $me->email ?? 'system',
    ...
    // Chiudi (di norma il ruolo assegnato o admin)
    ...
    // Registro attività (visibile a coordinamento; lato route proteggilo se vuoi)
    public function logs(Request $r)
    {
        ...
        if ($r->filled('segnalazione_id')) $q->where('segnalazione_id', (int)$r->integer('segnalazione_id'));
