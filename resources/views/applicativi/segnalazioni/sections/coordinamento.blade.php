...
<script type="module">
    ...
    function withDebug(path) {
        ...
        return path + (hasQuery ? '&' : '?') + 'debug_roles=1';
    }
    ...
    const state = {
        ...
        activeTab: 'queue',
        lists: {
            queue: { rows: [], meta: { current_page: 1, last_page: 1, total: 0 } },
            work:  { rows: [], meta: { current_page: 1, last_page: 1, total: 0 } },
            closed:{ rows: [], meta: { current_page: 1, last_page: 1, total: 0 } },
        },
        counts: {
            queue: 0,
            work:  0,
    ...
    function renderLegend() {
        ...
        mount.innerHTML = keys.map(t => `
            <span class="legend__item" data-type="${t}">
    ...
    function card(row) {
        ...
        const badge = g.status === 'queue'    ? 'b-queue'
                    : g.status === 'assigned' ? 'b-work'
                    : 'b-closed';
        const stxt  = g.status === 'queue'    ? 'In coda'
                    : g.status === 'assigned' ? 'In lavorazione'
                    : 'Chiusa';
        ...
        <div class="card__body">
            <div class="row">
                ...
                <div>
                    ...
                    <div>${
                        areas!=='—'
                            ? areas.split(',').map(a=>`<span class="tag">${a.trim()}</span>`).join(' ')
                            : '—'
                    }</div>
                </div>
            </div>
        ...
        root.querySelectorAll('.pager').forEach(pg => {
            ...
            pg.querySelectorAll('[data-pg]').forEach(btn => {
                btn.addEventListener('click', async () => {
                    ...
                    if (btn.dataset.pg === 'prev')
                        state.pages[kind] = Math.max(1, (meta.current_page || 1) - 1);
                    if (btn.dataset.pg === 'next')
                        state.pages[kind] = Math.min(meta.last_page || 1, (meta.current_page || 1) + 1);
    ...
    function render() {
        ...
        const effTab = (state.role !== 'coordinamento' && state.activeTab === 'queue')
            ? 'work'
            : state.activeTab;
    ...
    /* ===== MODALE: tutte le note ===== */
    async function loadAllNotesFor(id) {
        const out = [];
        let page = 1, per_page = 100, last = 1;
        do {
            const { data, meta } = await API.getNotes(id, page, per_page);
            out.push(...(data || []));
    ...
    async function promptInstructions() {
        if (!window.Swal) return { confirmed: true, text: '' };
        const { value: isText, isConfirmed } = await Swal.fire({
    ...
    async function loadList(kind) {
        ...
        if (kind === 'work') {
            ...
            if (!isCoord(state.role)) params.assigned_to = state.role;
        }
        if (kind === 'closed') {
    ...
    function makePrioBadge(p = 'Nessuna') {
        ...
        s.className = 'prio-badge ' + (
            p.toLowerCase() === 'alta'  ? 'prio--alta'  :
            p.toLowerCase() === 'media' ? 'prio--media' :
            p.toLowerCase() === 'bassa' ? 'prio--bassa' : 'prio--nessuna'
        );
    ...
    async function fetchSegnalazioneWithFallback(id) {
        ...
        const hit = ['queue', 'work', 'closed']
            .flatMap(k => state.lists[k]?.rows || [])
